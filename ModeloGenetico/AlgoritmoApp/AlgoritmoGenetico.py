...
def verificarCriterio(poblacion, generacion,criterio,archivo):
    ...
    #porcentaje de la poblacion con valor fitness aceptable        
        ...
        #si almenos el 50% de los individuos tienen un valor fitness menor a 1
        aceptables=0
        ...
        if aceptables > len(poblacion)/2:
            return True
    #Valor fitness promedio dentro de la poblacion
    elif criterio ==3:
        #si el promedio del fitness de la poblacion es como maximo 3
        sumaFitness=0
        for individuo in poblacion:
            individuo.fitness = evaluarFitness(individuo.solucion,archivo)
            sumaFitness=sumaFitness + individuo.fitness 
        if sumaFitness / len(poblacion) < 2:
            return True    
    return
...
def ejecutar(datos_archivo,criterio_fin,criterio_selecPadres,archivoNAME):
    ...
    if criterio_fin == "Maximo numero de generaciones":
        ...
        criterio_fin_INT = 2
    elif criterio_fin == "Un valor fitness promedio dentro de la poblacion":
        criterio_fin_INT = 3
